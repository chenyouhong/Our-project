...
from MTCL_UAV.layers.Embedding import *
import pdb
...
class Transformer_Layer(nn.Module):  
    ...
    def forward(self, x):  
        """  
        x: [batch_size, seq_len, vars, d_model]  
        """  
        new_x = x  
        batch_size = x.size(0)  
        ...
        for i in range(self.patch_nums):  
            t = x[:, i * self.patch_size:(i + 1) * self.patch_size, :, :]  # [batch, patch_size, vars, d_model]  
        ...
        intra_out_concat = self.intra_Linear(intra_out_concat)    # [batch, patch_nums * patch_size, vars, d_model]  
        intra_out_concat = intra_out_concat.permute(0, 3, 2, 1)

        #### Inter Attention ######  
        x = x.unfold(dimension=1, size=self.patch_size, step=self.stride)  # [batch, patch_num, vars, d_model, patch_size]  
        x = x.permute(0, 2, 1, 3, 4)  # [batch, vars, patch_num, d_model, patch_size]  
        b, nvar, patch_num, dim, patch_len = x.shape  


        x = torch.reshape(x, (x.shape[0] * x.shape[1], x.shape[2], x.shape[3] * x.shape[-1]))  # [batch * nvar, patch_num, dim * patch_len]  


        ed = self.euclidean_distance(x)
        w_sim = torch.exp(-ed / self.temp)

        x = self.emb_linear(x)  # [batch * nvar, patch_num, dim * patch_len]  

        x = self.dropout(x + self.W_pos)

        inter_out, attention = self.inter_patch_attention(Q=x, K=x, V=x)  # [batch * nvar, patch_num, d_model]  
        inter_out = torch.reshape(inter_out, (b, nvar, inter_out.shape[-2], inter_out.shape[-1]))  # [batch, nvar, patch_num, d_model]  
        inter_out = torch.reshape(inter_out, (b, nvar, inter_out.shape[-2], self.patch_size, self.d_model))  # [batch, nvar, patch_num, patch_size, d_model]  
        inter_out = torch.reshape(inter_out, (b, self.patch_size * self.patch_nums, nvar, self.d_model))  # [batch, temporal, nvar, d_model]  


        out = new_x + intra_out_concat + inter_out  # [batch, temporal, nvar, d_model]  
        if self.batch_norm:  
        ...
        outt = out.unfold(dimension=1, size=self.patch_size, step=self.stride)  # [batch, patch_num, nvar, d_model, patch_size]  
        outt = outt.permute(0, 2, 1, 3, 4)  # [batch, nvar, patch_num, d_model, patch_size]  
        outt = torch.reshape(outt, (outt.shape[0] * outt.shape[1], outt.shape[2], outt.shape[3] * outt.shape[-1]))  # [batch * nvar, patch_num, d_model * patch_size]  

        ed_out = self.euclidean_distance(outt)  # [batch * nvar, patch_num, patch_num]
        loss_similar = ed_out * w_sim  # 保证损失为非负值
        average_loss = loss_similar.mean()

        return out, attention, average_loss
...
class EuclideanDistance(nn.Module):  
    ...
    def forward(self, x):
        B, N, D = x.shape
        x_square = x.pow(2).sum(dim=2, keepdim=True)
        y_square = x_square.transpose(1, 2)
        xy = torch.bmm(x, x.transpose(1, 2))
        distance = torch.sqrt(torch.clamp(x_square + y_square - 2 * xy, min=self.eps))  

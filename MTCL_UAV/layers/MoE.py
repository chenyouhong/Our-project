...
from MTCL_UAV.layers.Layer import Transformer_Layer
from MTCL_UAV.utils.Other import SparseDispatcher, FourierLayer, series_decomp_multi, MLP
import pdb
import numpy as np
...
class MoE(nn.Module):
    def __init__(self, input_size, output_size, num_experts, device, num_nodes=1, d_model=32, d_ff=64, dynamic=False,
                 patch_size=[8, 6, 4, 2], noisy_gating=True, k=4, layer_number=1, residual_connection=1, batch_norm=False, temp=1,
                 ):
        ...
        for patch in patch_size:
            ...
            self.experts.append(Transformer_Layer(device=device, d_model=d_model, d_ff=d_ff,
                                      ...
                                      patch_size=patch, factorized=True, layer_number=layer_number, batch_norm=batch_norm, temp=temp,
                                      ))

        # self.w_gate = nn.Parameter(torch.zeros(input_size, num_experts), requires_grad=True)
        # self.w_noise = nn.Parameter(torch.zeros(input_size, num_experts), requires_grad=True)
        self.w_noise = nn.Linear(input_size, num_experts)
        self.w_noise_norm = nn.LayerNorm(num_experts)
        self.w_gate = nn.Linear(input_size, num_experts)
        self.w_gate_norm = nn.LayerNorm(num_experts)
    ...
    def noisy_top_k_gating(self, x, train, noise_epsilon=1e-2):
        ...
        clean_logits = self.w_gate_norm(self.w_gate(x))  
        if self.noisy_gating and train:  
            raw_noise_stddev = self.w_noise_norm(self.w_noise(x))  
    ...
    def forward(self, x, loss_coef=1e-2):
        ...
        for i in range(self.num_experts):
            outputs, _, weight = self.experts[i](expert_inputs[i])  
            expert_outputs.append(outputs)  
            loss_contrast.append(weight)  
        ...
        if torch.isnan(output).any():  
            print("Error: Tensor contains NaN values")  
            pdb.set_trace()
        contrastive_loss = torch.nanmean(torch.stack(loss_contrast))  

        return output, balance_loss, contrastive_loss
